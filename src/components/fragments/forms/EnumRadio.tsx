...
const EnumRadio = (props: Readonly<EnumRadioParams>) => {
  ...
  return (
    <FormGroup className={props.className}>
      ...
      {values.map((val, index) => <Form.Check key={index} value={val} inline type="radio" label={t(`${props.enumName}.${val}`)} className={props.innerClass} required={props.required} disabled={props.disabled} onChange={props.onChange} checked={val === props.value} />)}
